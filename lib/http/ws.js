    ...
    server.on('connection', function connection(ws) {
        ws.on('message', function incoming(msg, flags) {
            if (flags.binary) {
				...
				varerror = error.invalid('Binary not accepted');
            ...
            try {
				...
				error = error.invalid('Invalid JSON received');
                ws.send(JSON.stringify({
                    type: 'error',
                    error: error
                }), handleSend);
            ...
            if(!msg.eventName) {
				error = error.invalid('No eventName send');
                ws.send(JSON.stringify({
                    type: 'error',
                    error: error
                }), handleSend);
            ...
            if(msg.eventName == "auth") {
                if(!msg.data || !msg.data.token) {
					error = error.invalid('Missing data');
                    ws.send(JSON.stringify({
                        type: 'error',
						error: error
                    }), handleSend);
