    ...
    server.on('connection', function connection(ws) {
        ws.on('message', function incoming(msg, flags) {
            if (flags.binary) {
				...
				var error = error.invalid('Binary not accepted');
            ...
            try {
				...
				var error = error.invalid('Invalid JSON received');
                ws.send(JSON.stringify({
                    type: 'error',
                    error: error
                }), handleSend);
            ...
            if(!msg.eventName) {
				var error = error.invalid('No eventName send');
                ws.send(JSON.stringify({
                    type: 'error',
                    error: error
                }), handleSend);
            ...
            if(msg.eventName == "auth") {
                if(!msg.data || !msg.data.token) {
					var error = error.invalid('Missing data');
                    ws.send(JSON.stringify({
                        type: 'error',
						error: error
                    }), handleSend);
                ...
                if(msg.data.type == 'session') {
					...
					var error = error.denied('oAuth not implemented.');
                    ws.send(JSON.stringify({
                        type: 'error',
						error: error
                    }), handleSend);
